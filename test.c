...
void print_packets(WINDOW *win);
void process_packet(WINDOW *win, unsigned char *buffer, int size, int packet_no);
...
void tcp_trace(PacketInfo *packet, WINDOW *win);
...
int main() {
    ...
    while (1) {
        ...
        if (ch != ERR) {
            ...
            if (ch == KEY_UP) {
                ...
                if(packet_list[cursor_position]->protocol == PROTOCOL_TCP){
                    tcp_trace(packet_list[cursor_position], packet_win);
                }
                // Removed break; to continue capturing after tracing
            }
        }
        ...
        packet_no++;
        process_packet(packet_win, buffer, data_size, packet_no);
    }
...
void print_packets(WINDOW *win) {
    ...
    // when the cursor needs to go outside the box
...
void tcp_trace(PacketInfo *packet, WINDOW *win){
    ...
    // Prepare the list of packets in the same TCP stream
    for (int i = 0; i < packet_count; ++i) {
        if (is_same_tcp_stream(packet, packet_list[i])) {
    ...
    int ch;
    nodelay(win, FALSE);  // Make wgetch blocking
    while (1) {
        ...
        if (ch == KEY_UP) {
            ...
            if (cursor_position < stream_packet_count -1) {
                cursor_position++;
            }
        } else if (ch == 'q' || ch == 'Q') {
            break;  // Exit the TCP stream view
        }
    }

    // Return to non-blocking input mode
    nodelay(win, TRUE);
}
